...
const (
	...
	Ilen2     = 14       // 比1大一倍
	Imax2     = 16384    // 1<< Ilen //1024  262144
	Treelen2  = 32768    // imax*2 //2048  524288
	TestNum2  = 16384
)
...
func HomoBuildTree(zs []*FieldVal) [Treelen2]*FieldVal {
	var ZTree [Treelen2]*FieldVal
	for i := 0; i < Imax2; i++ {
		ZTree[i] = zs[i] //初始化叶子结点值
	}
	offset := Imax2
	treelen := Imax2*2 - 3 //计算树的节点总数
	//treelen1 := treelen - 1
	for i := 0; i < treelen; i += 2 { //构建分支节点
		z := new(FieldVal)
		zmult := z.Mul2(ZTree[i], ZTree[i+1]) //两个节点的父亲节点为两个节点的乘积
...
func HomoGetInvTree(rootinv *FieldVal, ZTree [Treelen2]*FieldVal) [Treelen2]*FieldVal { //逆树是从顶向下构建
	var ZinvTree [Treelen2]*FieldVal
	treelen := Imax2*2 - 2 //逆树的总节点数
...
func BatchHomoAdd(ciphers1, ciphers2 []*Cipher) []*Cipher {
	var x1, y1, x2, y2 []*big.Int
	// 将 Cipher 对象的数据提取为批量点加的输入
	...
	// 调用 BatchAdd 批量执行点加运算
	resx, resy := c.BatchAdd(x1, y1, x2, y2)

	// 构造新的 Cipher 结果对象
	var result []*Cipher
	for i := 0; i < len(resx); i += 2 {
		cipher := &Cipher{
			C1x: resx[i], C1y: resy[i],
			C2x: resx[i+1], C2y: resy[i+1],
		}
...
func BatchHomoMul(ciphers []*Cipher, ks []*big.Int) []*Cipher {

	// 准备输入数组
	Bx1 := make([]*big.Int, len(ciphers)*2)
	By1 := make([]*big.Int, len(ciphers)*2)
	kBytes := make([][]byte, len(ciphers))

	// 填充输入数组
	for i, c := range ciphers {
		Bx1[2*i], By1[2*i] = c.C1x, c.C1y
		Bx1[2*i+1], By1[2*i+1] = c.C2x, c.C2y
		kBytes[i] = ks[i].Bytes()
	}

	// 批量乘法计算
	resx, resy := c.ScalarMultBatchFieldValBatchQHomo(Bx1, By1, kBytes)
