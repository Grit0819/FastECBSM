...
// 利用逆树来进行还原
func (curve *KoblitzCurve) homofieldJacobianToBigAffineBatch(x, y []*FieldVal, ZinvTree [Treelen2]*FieldVal) ([]*big.Int, []*big.Int) {
	// Inversions are expensive and both point addition and point doubling
	// are faster when working with points that have a z value of one.  So,
	// if the point needs to be converted to affine, go ahead and normalize
	// the point itself at the same time as the calculation is the same.
...
// 批量加
func (curve *KoblitzCurve) BatchAdd(x1, y1, x2, y2 []*big.Int) ([]*big.Int, []*big.Int) {
	// A point at infinity is the identity according to the group law for
	// elliptic curve cryptography.  Thus, ∞ + P = P and P + ∞ = P.
	var resx, resy []*big.Int
	...
	// 遍历所有点并处理无穷远点的情况
	for i := 0; i < len(x1); i++ {
		// 如果第一个点是无穷远点，直接返回第二个点
		if x1[i].Sign() == 0 && y1[i].Sign() == 0 {
			resx = append(resx, x2[i])
			resy = append(resy, y2[i])
			continue
		}

		// 如果第二个点是无穷远点，直接返回第一个点
		if x2[i].Sign() == 0 && y2[i].Sign() == 0 {
			resx = append(resx, x1[i])
			resy = append(resy, y1[i])
			continue
		}

		// 将仿射坐标转换为域元素
		fx1, fy1 := curve.bigAffineToField(x1[i], y1[i])
		fx2, fy2 := curve.bigAffineToField(x2[i], y2[i])
	...
	// 如果有需要转换的雅可比坐标，则进行批量转换

	kTree := HomoBuildTree(Fz)
	treerootInv := new(FieldVal).Set(kTree[Treelen2-2].Inverse())
	ZinvTree := HomoGetInvTree(treerootInv, kTree)

	// 将雅可比坐标批量转换为仿射坐标
	resx, resy = curve.homofieldJacobianToBigAffineBatch(Fx, Fy, ZinvTree)
...
func (curve *KoblitzCurve) ScalarMultBatchFieldValBatchK(Bx, By *big.Int, ks [][]byte) ([]*big.Int, []*big.Int) {
...
func (curve *KoblitzCurve) ScalarMultBatchFieldValBatchQ(Bx, By []*big.Int, ks [][]byte) ([]*big.Int, []*big.Int) {
	...
	for i := 0; i < Imax; i++ {
		...
		Fy = append(Fy, new(FieldVal))
		Fz = append(Fz, new(FieldVal))
		Fx[i], Fy[i], Fz[i] = curve.scalarMult(Bx[i], By[i], ks[i])
	}
...
func (curve *KoblitzCurve) ScalarMultBatchFieldValBatchQHomo(Bx, By []*big.Int, ks [][]byte) ([]*big.Int, []*big.Int) {
	...
	for i := 0; i < Imax2; i++ {
		...
		Fy = append(Fy, new(FieldVal))
		Fz = append(Fz, new(FieldVal))
		Fx[i], Fy[i], Fz[i] = curve.scalarMult(Bx[i], By[i], ks[kIndex])
	}

	//使用trick
	kTree := HomoBuildTree(Fz) //构建树 k数组就是

	treeroot_inv := new(FieldVal).Set(kTree[Treelen2-2].Inverse()) //获取树的根节点，然后计算其逆元

	ZinvTree := HomoGetInvTree(treeroot_inv, kTree) //构建逆树
