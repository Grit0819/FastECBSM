...
func (curve *KoblitzCurve) ScalarMultBatchFieldVal(Bx, By []*big.Int, k []byte) ([]*big.Int, []*big.Int) {
	...
	// end2 := time.Now().UnixNano() // 使用纳秒级时间测量
	// costBatch2 := time.Duration(end2-start2) * time.Nanosecond
	// cost4 += costBatch2
	...
	for i := 0; i < repeatCount; i++ {
		...
		kTree := BuildTree(Fz)                                        // 构建树
		treeroot_inv := new(FieldVal).Set(kTree[Treelen-2].Inverse()) // 获取树根并计算逆元
		ZinvTree := GetInvTree(treeroot_inv, kTree)                   // 构建逆树
	...
	// 计算平均时间
	averageTime := totalTime / int64(repeatCount)
	costBatch2 := time.Duration(averageTime) * time.Nanosecond
	cost4 += costBatch2
	...
	fmt.Printf("\n")
	fmt.Printf("转化的用时优化率 %f\n", float64(cost2-cost4)/float64(cost2))
	fmt.Printf("总用时优化率 %f\n", float64(cost1+cost2-cost3-cost4)/float64(cost1+cost2))
	fmt.Printf("*******************************************\n")
	return resx, resy
