...
// 利用逆树来进行还原
func (curve *KoblitzCurve) afieldJacobianToBigAffineBatch(x, y []*FieldVal, ZinvTree []*FieldVal) ([]*big.Int, []*big.Int) {
	// Inversions are expensive and both point addition and point doubling
	// are faster when working with points that have a z value of one.  So,
	// if the point needs to be converted to affine, go ahead and normalize
	// the point itself at the same time as the calculation is the same.
...
func (curve *KoblitzCurve) ScalarMultBatchFieldVal(Bx, By []*big.Int, k []byte) ([]*big.Int, []*big.Int) {
	...
	kTree, sizes := ABuildTree(Fz) //构建树 k数组就是
	//treeroot_inv := new(FieldVal).Set(kTree[Treelen-2].Inverse()) //获取树的根节点，然后计算其逆元
	treeroot_inv := new(FieldVal).Set(kTree[len(kTree)-1].Inverse())
	//ZinvTree := GetInvTree(treeroot_inv, kTree) //构建逆树
	ZinvTree := AGetInvTree(treeroot_inv, kTree, sizes)
	//用逆树进行雅可比坐标的还原
	resx, resy = curve.afieldJacobianToBigAffineBatch(Fx, Fy, ZinvTree)
	return resx, resy
...
func (curve *KoblitzCurve) ScalarBaseMultBatch(k [][]byte) ([]*big.Int, []*big.Int) {
