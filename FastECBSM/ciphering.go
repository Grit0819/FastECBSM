...
const (
	Ilen      = 20      // 2的次方
	Imax      = 1048576 // 数量控制
	Treelen   = 2097152 // 数量的一倍
	TestNum   = 1048576 //测试消息数量控制
	Jlen      = 24
	Jmax      = 16777216
	cuckoolen = 21810381
)
...
func ABuildTree(zs []*FieldVal) ([]*FieldVal, []int) {
	...
	ZTree := append(make([]*FieldVal, 0, 2*n), zs...) // 优化：预先分配空间，减少扩容开销

	currentLevelSize := n
	var levelSizes []int // 记录每层节点数的数组
	...
	levelSizes = append(levelSizes, currentLevelSize)
