...
func (c *Op_Cuckoo) Op_insert(data [Jmax][]byte) { //(c *Op_Cuckoo)是指接受者的类型指定
	Hash_index := make([]uint8, c.maxsize) //动态创建一个切片

	for i := 0; i < c.nentries; i++ {
		v := Value(i + 1) //把v赋值为i+1的无符号32位
		old_hash_id := uint8(1)
		j := 0
		for ; j < MAXITER; j++ { //MAXITER = 500
			X := data[v-1]                 //把传入的二维数组中的x值 赋值给X
			start := (old_hash_id - 1) * 8 //相当于左移三位
			end := start + 4
			x := BytesToUint32(X[end : end+4]) //4字节转换为32位无符号hash值
			x_key := Key(x)
			h := BytesToUint32(X[start:end]) % c.maxsize
